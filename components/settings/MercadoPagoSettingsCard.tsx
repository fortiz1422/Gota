...
const diagnosticLabel = (value: string | null | undefined) => ({ expense: 'Gasto', income: 'Ingreso', transfer: 'Transferencia', neutral: 'Técnica', unknown: 'Desconocida', outflow: 'Salida', inflow: 'Entrada', internal: 'Interna', mercadopago_balance: 'Saldo MP', card: 'Tarjeta', bank_transfer: 'Cuenta externa', CHECKOUT: 'Checkout', INSTORE: 'Presencial', SUBSCRIPTIONS: 'Suscripciones', PSP_TRANSFER: 'Transferencia PSP', UNSPECIFIED: 'Sin canal' }[value ?? ''] ?? value ?? 'Sin dato')
function diagnosticFunding(funding: Diagnostic['fundingSource']) {
  ...
  return card ? `${diagnosticLabel(funding.kind)} · ${card}` : diagnosticLabel(funding.kind)
}
...
export function MercadoPagoSettingsCard() {
  ...
  const openReview = async (movement: Diagnostic) => {
    ...
    try {
      ...
      setAccounts(rows.filter((row): row is Account => typeof row.id === 'string' && typeof row.name === 'string' && (row.type === 'cash' || row.type === 'bank' || row.type === 'digital') && row.archived !== true).map(({ id, name, type, archived }) => ({ id, name, type, archived: Boolean(archived) })))
