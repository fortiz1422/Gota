  ...
  const result = await listSharedReceipts(user.id, {
    listPending: async (userId, statuses) => {
      const { data, error } = await admin
        ...
        .select('id,status,source_kind,source_app_hint,original_filename,mime_type,byte_size,parsed_payload,parser_version,parse_error_code,created_at,updated_at,expires_at')
        .eq('user_id', userId)
