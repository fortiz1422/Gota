...
describe('Account edit white pilot', () => {
  it('uses the approved compact chrome for account creation and editing', () => {
    ...
    expect(source).toContain("navigationTitle={isNew ? 'Nueva cuenta' : 'Editar cuenta'}")
  ...
  it('groups account edit controls into white semantic sections without tertiary field wells', () => {
    ...
    expect(source).toContain('const inputClass = editInputClass')
    expect(source).not.toContain('legacyInputClass')
