...
- `regular_payment` con rol payer es gasto; `recurring_payment` con rol payer es gasto recurrente; `money_transfer` con rol payer y PSP_TRANSFER es transferencia saliente.
...
- La fuente de fondos se mantiene separada del canal: saldo MP, tarjeta o transferencia bancaria/DEBIN. Sólo se conserva marca, issuer id y últimos cuatro de tarjeta cuando son valores seguros.
