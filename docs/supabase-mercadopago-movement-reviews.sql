...
-- Replace every known/obsolete arity on re-application. The current contract is
-- deliberately explicit: no caller-controlled expense or review fields.
...
begin
  ...
  -- Deterministic lock order: connection/user, RAW observations, account,
  -- then the existing review/idempotency reservation.
  ...
  for v_expected in select value from jsonb_array_elements(p_expected_observations) loop
    ...
    perform 1 from public.mercadopago_raw_observations r
     where r.id = (v_expected->>'id')::uuid and r.user_id = p_user_id
       ...
       and r.last_seen_at >= now() - interval '7 days'
     for update;
    if not found then raise exception 'missing, foreign or stale evidence' using errcode = 'P0002'; end if;
  end loop;
  select * into v_account from public.accounts where id = p_account_id
    and user_id = p_user_id and archived = false for update;
  ...
  if found then
    ...
    if v_review.evidence_kind <> p_evidence_kind or v_review.account_id <> p_account_id
       ...
       or v_review.canonical_description <> p_description then
  ...
  insert into public.expenses(user_id, amount, currency, category, description,
    payment_method, account_id, date)
  values (p_user_id, p_amount, p_currency, p_category, p_description,
    case v_account.type when 'cash' then 'CASH' else 'DEBIT' end,
  ...
  insert into public.mercadopago_movement_reviews(
    ...
    canonical_category, canonical_description, canonical_semantics, evidence, evidence_kind)
  values (p_user_id, p_connection_id, p_candidate_id, p_candidate_fingerprint,
    ...
    p_category, p_description, p_canonical_semantics,
    jsonb_build_object('observations', p_expected_observations), p_evidence_kind);
  return v_expense_id;
end;
$$;

revoke all on function public.confirm_mercadopago_expense(uuid,uuid,text,text,text,jsonb,numeric,text,date,text,text,uuid,text,jsonb) from public, anon, authenticated;
grant execute on function public.confirm_mercadopago_expense(uuid,uuid,text,text,text,jsonb,numeric,text,date,text,text,uuid,text,jsonb) to service_role;
